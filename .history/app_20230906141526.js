...
function newTask() {
  ...
  if (IsEmpty(userInput) && dueInput >= todaysDate) {
    // not empty
    let fullDate = new Intl.DateTimeFormat('en-GB', { dateStyle: 'full', timeStyle: 'long', timeZone: 'Australia/Sydney' }).format(DueDate);

    const todo = { id: Date.now(), task: userInput, due: dueInput };
